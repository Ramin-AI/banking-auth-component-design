...
// animated svg logout button with walking/falling animation
...
function applyState(buttonEl, stateName) {
  const stateProps = logoutButtonStates[stateName];
  if (!buttonEl || !stateProps) return;
  Object.entries(stateProps).forEach(([prop, value]) => {
    buttonEl.style.setProperty(prop, value);
  });
}

const AnimatedLogoutButton = ({ onLogout, theme: themeProp }) => {
  const themeContext = useContext(ThemeContext);
  const theme = themeProp || (themeContext && themeContext.theme) || 'dark';

  const [animState, setAnimState] = useState('default');
  ...
  const handleMouseEnter = useCallback(() => {
    if (animState === 'default') {
      setAnimState('hover');
      applyState(buttonRef.current, 'hover');
    }
  }, [animState]);
  ...
  const handleMouseLeave = useCallback(() => {
    if (animState === 'hover') {
      ...
      applyState(buttonRef.current, 'default');
    }
  }, [animState]);

  const handleClick = useCallback(() => {
    if (animState !== 'default' && animState !== 'hover') return;
    ...
    setAnimState('walking1');

    // Step 2: walking2 after 300ms
    scheduleTimer(() => {
      btn.classList.add('door-slammed');
      applyState(btn, 'walking2');
      setAnimState('walking2');

      // Step 3: falling1 after 400ms
      scheduleTimer(() => {
        btn.classList.add('falling');
        applyState(btn, 'falling1');
        setAnimState('falling1');

        // Step 4: falling2 after 400ms
        scheduleTimer(() => {
          applyState(btn, 'falling2');
          setAnimState('falling2');

          // Step 5: falling3 after 300ms
          scheduleTimer(() => {
            applyState(btn, 'falling3');
            setAnimState('falling3');

            // Step 6: call onLogout after 1000ms
            scheduleTimer(() => {
  ...
  }, [animState, onLogout, clearTimers, scheduleTimer]);

  const themeClass = theme === 'light' ? 'logoutButton--light' : 'logoutButton--dark';

  return (
    <div className="animated-logout-wrapper">
      <button
        ref={buttonRef}
        className={`logoutButton ${themeClass}`}
